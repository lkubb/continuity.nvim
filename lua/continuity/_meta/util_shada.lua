---@meta
---@namespace continuity.util.shada

--- Influence `Shada:read` behavior
---@class ReadOpts
---@field bang? boolean Overwrite this instance's Shada-sourced data with the contents in this builder. Defaults to false.
...
---@field [3]? integer Single byte value of separator char. Only for search type. E.g. `47` (ascii byte of /) for / search entry
...
---@field n integer Single byte value of the mark name (ASCII char, uppercase). In theory defaults to 34 (= `"`, unnamed register).
...
---@field n integer Single byte value of the mark name (ASCII char, lowercase). In theory defaults to 34 (= `"`, unnamed register).
...
---@alias Entry.Header ShadaEntry<1, EntryData.Header> Header entry. Of no functional value, just for debugging.
---@alias Entry.SearchPattern ShadaEntry<2, EntryData.SearchPattern> Recent search pattern. Usually 2 per ShaDa, (1) `ss` = `true` (2) `ss` = `false`. One of the entries should have `su` == `true`.
---@alias Entry.SubString ShadaEntry<3, EntryData.SubString> Last `:s[ubstitute]` replacement string
---@alias Entry.History ShadaEntry<4, EntryData.History> One line of (`cmd`|`search`|`expr`|`input`|`debug`) history
---@alias Entry.Register ShadaEntry<5, EntryData.Register> Named register.
---@alias Entry.Variable ShadaEntry<6, EntryData.Variable> Global (`UPPERCASE_ONLY` but not `_WITHOUT_LEADING_UPPERCASE`) variable.
---@alias Entry.GlobalMark ShadaEntry<7, EntryData.GlobalMark> Global (uppercase) mark, including `'0-'9`.
---@alias Entry.Jump ShadaEntry<8, EntryData.Jump> Jumplist item of last active window (named file buffers only).
---@alias Entry.BufferList ShadaEntry<9, EntryData.BufferList> List of loaded buffers and recent cursor positions.
---@alias Entry.LocalMark ShadaEntry<10, EntryData.LocalMark> Buffer-local (lowercase) mark of some recently active buffer.
---@alias Entry.Change ShadaEntry<11, EntryData.Change> Changelist item of some recently active named file buffer.
