...
function M.get_tab(tabid, opts)
  ...
  -- tracks the option's value per tabpage (setup in `plugin/finni.lua`).
  if vim.tbl_contains(opts, "cmdheight") then
    ret.cmdheight = vim.t[tabid].finni_cmdheight_tracker or vim.o.cmdheight
  end
  return ret
end
...
function M.with(overrides, inner, ...)
  ...
  return require("finni.util").try_finally(inner, function()
